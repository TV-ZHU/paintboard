...
var lastActionTime;
var actionWs, actionCurrentMap, actionCurrentWantedMap, actionTotal, actionCurrentFixed, actionAttack, actionMovement, actionMovementFails;
...
function actionTouch(tm){
	...
	actionInterval = setInterval(function(){
		...
		paintPixel(tk[0], px[1], px[0], Number.parseInt(actionCurrentWantedMap[px[0]][px[1]], 32), function(can){
			if(can){
				++ actionMovement;
				tk[1] = Date.now();
			}
...
function updateColor(x, y, col, tm){
	if(actionCurrentWantedMap[x][y] == '-')
		return;
	var X = false, Y = false;
	X = actionCurrentWantedMap[x][y] == actionCurrentMap[x][y];
	Y = actionCurrentWantedMap[x][y] == col;
	actionCurrentMap[x][y] = col;
	actionCurrentFixed = actionCurrentFixed - X + Y;
	if(X && !Y){
		actionWaitList.push([x, y]);
		actionTouch(tm);
	}
	actionAttack += !Y;
	flushAction();
...
function mainFunc(tm, clr = true){
	...
	actionWs.onmessage = function(evt){
		...
		if(data.type === "paintboard_update") {
            updateColor(data.y, data.x, data.color, tm);
